...
class WorkerMonitor(actorNum: Int, MaxGossipToHear: Int, MaxSimilarNumToCal: Int, algm: String, topo: String, actorInterval:Int, pushsumStratedy:String, showlog:Boolean) extends Actor {
...
//      println("~~~~~~~~~~~~"+sender())
  ...
  def startActors = {
    for (i <- 0 until actorNum) {
      var actor = context.system.actorOf(Props(classOf[Worker],actorInterval,pushsumStratedy, showlog,self), name = "actor" + i)
      actorpool += actor;
    }
